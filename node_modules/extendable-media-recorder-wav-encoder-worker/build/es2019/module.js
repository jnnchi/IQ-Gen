...
createWorker(self, {
    ...
    encode: ({ recordingId, timeslice }) => {
        ...
        if (timeslice !== null) {
            ...
            const shiftedChannelDataArrays = shiftChannelDataArrays(recording.channelDataArrays, Math.ceil(timeslice * (recording.sampleRate / 1000)));
    ...
    record: ({ recordingId, sampleRate, typedArrays }) => {
        ...
        if (encoding !== undefined && computeNumberOfSamples(recording.channelDataArrays[0]) * (1000 / sampleRate) >= encoding.timeslice) {
            const shiftedChannelDataArrays = shiftChannelDataArrays(recording.channelDataArrays, Math.ceil(encoding.timeslice * (sampleRate / 1000)));
